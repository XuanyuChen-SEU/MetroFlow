...
def _build_trend(macro_df: pd.DataFrame, city_name: str = "上海") -> list[dict]:
    national = (
    ...
    city_df = (
        macro_df.loc[macro_df["city_name"] == city_name, ["stat_date", "avg_daily_flow"]]
        .rename(columns={"avg_daily_flow": "city_flow"})
    )
    trend = national.merge(city_df, on="stat_date", how="left").sort_values("stat_date")
    ...
    trend["city_flow"] = trend["city_flow"].fillna(0)
    return trend[["label", "national_flow", "city_flow", "intensity_index"]].round(2).to_dict(orient="records")
...
def get_overview() -> dict:
    ...
    top10 = (
        ...
        .head(10).rename(columns={"avg_daily_flow": "value"}).to_dict(orient="records")
    )
    meta = {
    ...
    return {"metrics": metrics, "top10": top10, "trend": _build_trend(macro_df, "上海"), "meta": meta}

def get_city_trend(city_name: str) -> list[dict]:
    macro_df = _normalize_macro_df(store.macro_city())
    return _build_trend(macro_df, city_name)
